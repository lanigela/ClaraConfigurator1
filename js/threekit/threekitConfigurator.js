...
class ThreekitConfigurator {
  ...
  _setupConfigurator(clara) {
    ...
    window.clara.attachCheckoutHander(function(ajax = false) {
      ...
      self.currentConfigFoamVolume = self
        ...
        .toFixed(2);
      self.currentConfigFabricArea = self
  ...
  _generatePostData() {
        ...
        switch (configType.get(attr)) {
          ...
          case 'Options':
            ...
            if (
              ...
              (attr === 'Leather Options' &&
                config['Cover Material'] === 'Fabric')
            ) {
              break;
            }
    ...
    var volumeOptionId = map
      .get('Foam_Volume_Price')
      .get('options')
      .get(materialPrice)
      .get('id');
    result['bundle_option[' + volumeId + ']'] = volumeOptionId;
    result[
      'bundle_option_qty[' + volumeId + ']'
    ] = this.currentConfigFoamVolume;
  ...
  _getBackPuffyThickness(config) {
    ...
    if (width < height) {
      thicknessA =
        defaultThicknessBackPuffy * (width / (defaultDimensions.Width / 100));
    } else {
      thicknessA =
        defaultThicknessBackPuffy * (height / (defaultDimensions.Height / 100));
    }
    return thicknessA;
  }
  ...
  _getCushionDimensions(config) {
    ...
    var shape =
      cushionType === 'Back (Angled)'
        ? config['Shape (Angled Back)']
        : config['Shape'];
  ...
  _getFoamVolume(config) {
    ...
    var volume = Object.keys(dimensions).reduce(function(
      result,
      dimensionName
    ) {
    ...
    // thickness
    if (config['Pillow Type'] === 'Back (Puffy)')
      volume *= this._getBackPuffyThickness(config);
  ...
  _getFabricArea(config, unit = 'dm2') {
    ...
    var maxThickness =
      config['Pillow Type'] === 'Back (Puffy)'
        ? this._getBackPuffyThickness(config)
        : dimensions.thickness.reduce(function(min, value) {
            return Math.max(min, Number(config[value]) / 100);
          }, 0);

    var depthDimName =
      config['Pillow Type'] === 'Bottom' ||
      config['Pillow Type'] === 'Bottom (Puffy)'
        ? 'depth'
        : 'height';
    ...
    // return dm^2
    return (
      maxWidth * maxThickness * 2 +
      maxWidth * maxDepth * 2 +
      maxThickness * maxDepth * 2 * 100
    );
  }
